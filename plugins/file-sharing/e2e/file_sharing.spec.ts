...
test.describe.serial("Cockpit File Sharing Plugin Comprehensive E2E Suite", () => {
  ...
  test("2. Full SMB Share CRUD Workflow & Ansible Lock Check", async () => {
    const frame = await getFrame();
    await frame.locator("button.pf-v6-c-tabs__link:has-text('SMB Shares'), [role='tab']:has-text('SMB Shares')").first().click();
    ...
    await expect(frame.locator(".pf-v6-c-modal-box__title-text")).toBeVisible({ timeout: 5000 });
  ...
  test("7. Verify Modal Background Dimming and Edge Fadeout", async () => {
    ...
    const modalTitle = frame.locator(".pf-v6-c-modal-box__title-text");
  ...
  test("10. Full NFS Export CRUD Workflow", async () => {
    const frame = await getFrame();
    await frame.locator("button.pf-v6-c-tabs__link:has-text('NFS Exports'), [role='tab']:has-text('NFS Exports')").first().click();
  ...
  test("11. Samba User Lifecycle & Password Management", async () => {
    const frame = await getFrame();
    await frame.locator("button.pf-v6-c-tabs__link:has-text('Samba Users'), [role='tab']:has-text('Samba Users')").first().click();
    ...
    const saveUserBtn = frame.locator(".pf-v6-c-modal-box button:has-text('Add user')").first();
    ...
    if (await userToggle.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await pwdOption.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        await frame.locator(".pf-v6-c-modal-box button:has-text('Update password')").first().click();
        await expect(frame.locator(".pf-v6-c-modal-box")).toHaveCount(0, { timeout: 10000 });
      }
    }
    ...
    if (await userToggle2.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await delOption.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const confirmDel = frame.locator(".pf-v6-c-modal-box button:has-text('Delete user')").first();
  ...
  test("12. Global Settings and Ansible Markers Modification", async () => {
    const frame = await getFrame();
    await frame.locator("button.pf-v6-c-tabs__link:has-text('Settings'), [role='tab']:has-text('Settings')").first().click();
  ...
  test("13. Services Restart and Reload Management", async () => {
    const frame = await getFrame();
    await frame.locator("button.pf-v6-c-tabs__link:has-text('Services & Sessions'), [role='tab']:has-text('Services')").first().click();
  ...
  test("16. Sessions and Service Actions UI Suite", async () => {
    const frame = await getFrame();
    await frame.locator("button.pf-v6-c-tabs__link:has-text('Services & Sessions'), [role='tab']:has-text('Services & Sessions')").first().click();
    await page.waitForTimeout(300);
  ...
  test("17. Comprehensive UI View Traversal & Modals Suite", async () => {
    ...
    if (await addShareBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await frame.locator(".pf-v6-c-modal-box button:has-text('Create share'), .pf-v6-c-modal-box button:has-text('Save share')").first().click({ timeout: 1000 }).catch(() => {});
      ...
      const submitShareModal = frame.locator(".pf-v6-c-modal-box button:has-text('Create share'), .pf-v6-c-modal-box button:has-text('Save share')").first();
      if (await submitShareModal.isVisible({ timeout: 1000 }).catch(() => false)) {
        await submitShareModal.click({ timeout: 1000 }).catch(() => {});
        await page.waitForTimeout(300);
      }

      const cancelModal = frame.locator(".pf-v6-c-modal-box button:has-text('Cancel')").first();
    ...
    if (await addExportBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      // Trigger empty validation
      await frame.locator(".pf-v6-c-modal-box button:has-text('Create export'), .pf-v6-c-modal-box button:has-text('Save export')").first().click({ timeout: 1000 }).catch(() => {});
      await page.waitForTimeout(100);
      ...
      const submitNfsModal = frame.locator(".pf-v6-c-modal-box button:has-text('Create export'), .pf-v6-c-modal-box button:has-text('Save export')").first();
    ...
    if (await addUserBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      // Trigger empty validation
      await frame.locator(".pf-v6-c-modal-box button:has-text('Add user')").first().click({ timeout: 1000 }).catch(() => {});
      await page.waitForTimeout(100);
      ...
      const submitUserModal = frame.locator(".pf-v6-c-modal-box button:has-text('Add user')").first();
  ...
  test("18. NFS Client Mapping & Samba Access Matrix Subtabs", async () => {
    ...
    await page.waitForTimeout(300);
    const clientsSubTab = frame.locator("button.pf-v6-c-tabs__link:has-text('Client IP Access Map'), [role='tab']:has-text('Client IP')").first();
    if (await clientsSubTab.isVisible({ timeout: 2000 }).catch(() => false)) {
      await clientsSubTab.click({ timeout: 2000 }).catch(() => {});
      await page.waitForTimeout(300);
    }
    const exportsSubTab = frame.locator("button.pf-v6-c-tabs__link:has-text('Export Paths'), [role='tab']:has-text('Export Paths')").first();
    ...
    await page.waitForTimeout(300);
    const matrixSubTab = frame.locator("button.pf-v6-c-tabs__link:has-text('User Access Matrix'), [role='tab']:has-text('User Access Matrix')").first();
    if (await matrixSubTab.isVisible({ timeout: 2000 }).catch(() => false)) {
      await matrixSubTab.click({ timeout: 2000 }).catch(() => {});
      await page.waitForTimeout(300);
    }
    const usersSubTab = frame.locator("button.pf-v6-c-tabs__link:has-text('Samba Users'), [role='tab']:has-text('Samba Users')").first();
  ...
  test("23. Table Row Action Menus and Modals Execution", async () => {
    ...
    if (await userKebab.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      await page.waitForTimeout(200);
      const setPassItem = frame.locator("button.pf-v6-c-menu__item:has-text('Set password')").first();
      if (await setPassItem.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        if (await newPass.isVisible({ timeout: 1000 }).catch(() => false)) {
          ...
          const updateBtn = frame.locator(".pf-v6-c-modal-box button:has-text('Update password')").first();
      ...
      await userKebab.click({ timeout: 1000 }).catch(() => {});
      await page.waitForTimeout(200);
      const deleteUserItem = frame.locator("button.pf-v6-c-menu__item:has-text('Delete Samba user')").first();
      if (await deleteUserItem.isVisible({ timeout: 1000 }).catch(() => false)) {
        await deleteUserItem.click({ timeout: 1000 }).catch(() => {});
        await page.waitForTimeout(200);
        const delBtn = frame.locator(".pf-v6-c-modal-box button:has-text('Delete Samba user')").first();
    ...
    if (await smbKebab.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      await page.waitForTimeout(200);
      const editItem = frame.locator("button.pf-v6-c-menu__item:has-text('Edit share')").first();
      if (await editItem.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        const saveChangesBtn = frame.locator(".pf-v6-c-modal-box button:has-text('Save changes')").first();
      ...
      await smbKebab.click({ timeout: 1000 }).catch(() => {});
      await page.waitForTimeout(200);
      const deleteShareItem = frame.locator("button.pf-v6-c-menu__item:has-text('Delete share')").first();
      if (await deleteShareItem.isVisible({ timeout: 1000 }).catch(() => false)) {
        await deleteShareItem.click({ timeout: 1000 }).catch(() => {});
        await page.waitForTimeout(200);
        const delBtn = frame.locator(".pf-v6-c-modal-box button:has-text('Delete share')").first();
        if (await delBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
          await delBtn.click({ timeout: 1000 }).catch(() => {});
          await page.waitForTimeout(200);
        }
    ...
    if (await nfsKebab.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      await page.waitForTimeout(200);
      const editNfsItem = frame.locator("button.pf-v6-c-menu__item:has-text('Edit export')").first();
      if (await editNfsItem.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        const saveChangesBtn = frame.locator(".pf-v6-c-modal-box button:has-text('Save changes'), .pf-v6-c-modal-box button:has-text('Save export')").first();
        if (await saveChangesBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
          await saveChangesBtn.click({ timeout: 1000 }).catch(() => {});
          await page.waitForTimeout(200);
        }
      ...
      await nfsKebab.click({ timeout: 1000 }).catch(() => {});
      await page.waitForTimeout(200);
      const deleteExportItem = frame.locator("button.pf-v6-c-menu__item:has-text('Delete export')").first();
      if (await deleteExportItem.isVisible({ timeout: 1000 }).catch(() => false)) {
        await deleteExportItem.click({ timeout: 1000 }).catch(() => {});
        await page.waitForTimeout(200);
        const delBtn = frame.locator(".pf-v6-c-modal-box button:has-text('Delete export')").first();
        if (await delBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
          await delBtn.click({ timeout: 1000 }).catch(() => {});
          await page.waitForTimeout(200);
        }
  ...
  test("26. User Access Matrix & Direct User Management Suite", async () => {
    ...
    if (await addUserBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      if (await sel.isVisible({ timeout: 1000 }).catch(() => false)) {
        if (await sel.evaluate((el) => el.tagName.toLowerCase() === "select")) {
          ...
          await sel.fill("alice", { timeout: 1000 }).catch(() => {});
        }
      }
      const passInp = frame.locator("input#add-password").first();
      if (await passInp.isVisible({ timeout: 1000 }).catch(() => false)) {
        await passInp.fill("Password123!", { timeout: 1000 }).catch(() => {});
      }
      const confInp = frame.locator("input#add-confirm-password").first();
      if (await confInp.isVisible({ timeout: 1000 }).catch(() => false)) {
        await confInp.fill("Password123!", { timeout: 1000 }).catch(() => {});
      }
      const submitAdd = frame.locator(".pf-v6-c-modal-box button:has-text('Add user')").first();
    ...
    if (await userKebab.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      await userKebab.click({ timeout: 1000 }).catch(() => {});
      await page.waitForTimeout(200);
      const toggleItem = frame.locator("button.pf-v6-c-menu__item:has-text('Disable user'), button.pf-v6-c-menu__item:has-text('Enable user')").first();
